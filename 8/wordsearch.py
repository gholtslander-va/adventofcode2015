...
def string_counter(imp):
    """
    Count crap in a string ffs
    """
    # r"\x27" len is 4.
    # Add 4 for \" on each side.
    # Add 1 for \
    # Add 2 for outside ""
    ...
    # =====
    # 8 + 4 + 1 + 1 + 2 = 16
    num_slashes = imp.count('\\')
    num_quotes = imp.count('"')
    extraneous_additions = 6

    return len(imp) + num_slashes + num_quotes + extraneous_additions

    # charcode = len(imp) + 2
    # if imp.count('\\x') > 0:
    #     charmem = len(imp)
    #     # Unicode specific logic here
    #     return (charcode - charmem) * 2 + charcode + imp.count('\\x')
    # else:
    #     charmem = len(imp.decode('unicode_escape'))
    #
    # return (charcode - charmem) * 2 + charmem


assert string_counter(r"") == 6, 'You blew it'
assert string_counter(r"abc") == 9, 'You blew it'
assert string_counter(r"aaa\"aaa") == 16, 'You blew it'
assert string_counter(r"\x27") == 11, 'You blew it'
...
for string in testinput:
    charcode = len(string) + 2
    charmemory = string_counter(string)
    totalcharcode += charcode
    totalcharmemory += charmemory

print totalcharcode, totalcharmemory, totalcharencoded, totalcharmemory - totalcharcode
